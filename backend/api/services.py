...
def create_optimized_pdf(original_pdf_path: str) -> tuple[str, dict]:
    """
    ...
    try:
        ...
        page_mapping = {
            new_idx + 1 : original_idx + 1
            for new_idx, original_idx in enumerate(sorted_pages)
        }
    ...
    def _process_document_task(self, document_id: int):
        ...
        try:
            ...
            try:
                ...
                optimized_pdf_path, page_mapping = create_optimized_pdf(document.file.path)
                logger.info(f"Optimized PDF created. Mapping sizes: {len(page_mapping)}")
            except Exception as e:
            ...
            try:
                ...
                if optimized_pdf_path != document.file.path:
                    ...
                    raise e
            def remap_pages(data, mapping):
                if not mapping: return data
                
                if isinstance(data, dict):
                    # Nếu gặp object có trường "page", sửa nó
                    if "page" in data and isinstance(data["page"], int):
                        old_page = data["page"]
                        # Lấy trang gốc từ mapping, nếu không có thì giữ nguyên
                        new_page = mapping.get(old_page, old_page)
                        data["page"] = new_page
                    
                    # Đệ quy cho các trường con
                    for key, value in data.items():
                        remap_pages(value, mapping)
                        
                elif isinstance(data, list):
                    for item in data:
                        remap_pages(item, mapping)
                
                return data
            if page_mapping:
                logger.info("Remapping extracted page numbers to original PDF...")
                remap_pages(extracted_data, page_mapping)
